...
const explainMistake = async (req, res) => {
  try {
    const { questionTitle, prompt, studentAnswer, correctAnswer, unit, mistakeType, steps } = req.body;
    ...
    switch (mistakeType) {
      case 'DECIMAL_SLIP_10X':
        pedagogicalExplanation = `You answered ${studentAnswer} ${unit}, which is a 10-fold or 100-fold displacement from the verified educational result of ${correctAnswer} ${unit}. In nursing pharmacology, naked decimals (.X) or trailing zeros (X.0) are leading causes of fatal tenfold dosing errors.`;
      ...
      case 'UNIT_CONVERSION_ERROR':
        pedagogicalExplanation = `A metric conversion divergence occurred. Before calculating, all quantities must be expressed in the exact same metric unit (e.g. converting grams to milligrams or micrograms).`;
        remediationGuidance = '1 g = 1,000 mg | 1 mg = 1,000 mcg | 1 kg = 1,000 g | 1 L = 1,000 mL. Keep dimensional units aligned.';
        break;

      default:
        pedagogicalExplanation = `Your entered calculation (${studentAnswer} ${unit}) differed from the expected resolution (${correctAnswer} ${unit}). Review the Desired over Have formula: (D ÷ H) × V. Ensure you placed the ordered dose in the numerator and available stock in the denominator.`;
